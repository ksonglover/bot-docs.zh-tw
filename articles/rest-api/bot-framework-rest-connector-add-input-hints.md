...
若要指出您的 Bot 是被動接受輸入，但不等候使用者回應，請將代表訊息的 [Activity][] 物件內的 `inputHint` 屬性設定為 **acceptingInput**。 在許多通道上，這會啟用用戶端的輸入方塊並關閉麥克風，但使用者仍可存取。 例如，如果使用者按住 [麥克風] 按鈕，Cortana 會開啟麥克風接受使用者輸入。 
