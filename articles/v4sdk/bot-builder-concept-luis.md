...
每當整合 LUIS 的 Bot 收到語句，Bot 會將它傳送至 LUIS 應用程式，其會傳回包含意圖和實體的 JSON 回應。 Bot 建立器 SDK 會提供功能 (實作為[中介軟體](bot-builder-concept-middleware.md)) 以自動處理 LUIS 的回應，並將其傳遞至您的 Bot。 您可以使用 Bot _回合處理常式_中的[回合內容](bot-builder-concept-activity-processing.md#turn-context)，根據 LUIS 回應中的意圖來路由傳送對話流程。 
...
LUIS 應用程式提供功能，可藉由檢閱使用者傳送的訊息，改善應用程式效能。 請參閱[檢閱建議語句](https://docs.microsoft.com/azure/cognitive-services/LUIS/label-suggested-utterances)的逐步解說。
