...
在 `dialogs/mainDialog/index.js` 檔案中，更新建構函式以接受 `userState` 作為第二個引數。 然後從 `conversationState` 建立 `topicStates` 屬性，並從 `userState` 建立 `userProfile` 屬性。
